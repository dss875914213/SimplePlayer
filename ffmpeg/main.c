...
static double compute_target_delay(double delay, VideoState* is)
{
	...
	if (get_master_sync_type(is) != AV_SYNC_VIDEO_MASTER)
	{
		...
		if (!isnan(diff) && fabs(diff) < is->max_frame_duration)
		{
			...
			else if (diff >= sync_threhold && delay > AV_SYNC_FRAMEDUP_THRESHOLD)
				delay += diff;
	...
	for (;;)
	{
		...
		case SDL_MOUSEMOTION:
			...
			else
			{
				...
				mm = (ns % 3600) / 60;
				ss = ns % 60;
				av_log(NULL, AV_LOG_INFO, "Seek to %2.0f%% (%2d:%2d:%2d) of total duration (%2d:%2d:%2d)    \n", frac * 100,
...
int main(int argc, char* argv[])
{
	...
	if(InitSDL(audio_disable, display_disable, alwaysontop, borderless, renderer_info)==-1)
		do_exit(NULL);
